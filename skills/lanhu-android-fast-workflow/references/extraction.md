...
`image_review.ps1 -Capabilities` 可只读查看参数范围，不读取图片或生成文件。sheet 的 PageSize 默认 12、范围 1–24；按 coverage.next_page 分页，不通过增大到 36 等非法参数规避分页。批量调用逐条检查失败，不用最后一条成功覆盖前面错误。
...
- 缓存按内容、参数及脚本哈希，核验衍生图哈希后复用；摘要返回 previews、coverage（页数/下一页/未覆盖支持文件/跳过文件）和 manifest；缓存命中也保留这些信息。编号只是当次顺序，跨轮用来源路径与 sha256 识别；未支持格式不计入已查看，只在同一短表更新实际看过的区域，不把 generated_not_visually_reviewed 当验收。
