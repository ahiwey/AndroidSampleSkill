...
- 已授权设备验证时，在耗时构建前先做低成本前置检查：设备可用、当前账号/游客身份、入口所需资料与能力、目标外设连接状态。数据库里存在某个用户资料不证明当前身份能读取它；按真实查询条件核实，报告只保留结论，不输出账号或健康数据。入口缺失先核对业务条件，别反复导航或直接改权限/exported 来强开页面。
- 真实数据上的保存测试先确定最小临时数据和恢复范围，沿用已有授权；需要用户补测试身份时问一次，继续不依赖它的工作。原账号资料、全量数据库及无关设置不作为可随意重置的测试夹具。续验前重新取基线，区分文件不存在与读取失败，并校验备份格式，不能将命令错误文本当备份。恢复只撤销本次测试的行/key，保留期间其他变化，核对无关数据及重启后入口；记录“未写入/已恢复/尚未恢复”，不能只备份就声称已恢复。
- 设置弹框用真实宿主验证一条“打开 → 修改 → 取消不落库 → 再打开 → 保存回读”的代表链路，并抽查键盘与底部确认按钮。自定义日历/图表的触点从 UI 节点边界和实际命中规则确定；若坐标来源无法证明，保留该交互未验证，截图不能替代命中测试。
...
- 日期驱动的图形故障用实际失败日期转成固定 fixture，覆盖当前问题的边界位置（如末行标记），不要只重复原设计日期。返回/弹框动画结束后再留稳定截图，过渡帧不当最终视觉证据。
...
- UI tree 获取/解析失败先保存首个错误并区分命令失败、XML 提取和页面状态；原因明确后最多一次针对性重试。同一宿主持续失败时不逐弹框重试或轮换 dump 命令。已有键盘导航支持时，可用 Tab/Shift+Tab/Enter 操作真实控件，逐步核对焦点、稳定画面与保存结果；重绘/返回后重新确认焦点，不盲用旧序号。该方式不证明触点命中、软键盘布局或手势。无可靠路径的具体交互保留未验证，工具故障不等同业务失败；截图不替代未执行的保存链路。
...
同一生成目录的构建、同一设备的安装与交互按会话串行安排；一次空闲检查不是独占保证。已有项目协调渠道/锁可用时记录执行者、源码/产物与释放条件，不新建调度框架或未经授权联系其他任务。不能建立稳定使用窗口时继续独立静态工作，保留该阶段未验证。

第一处失败先区分本次源码、其他进行中源码、生成目录争用和未知原因；只有原因变化的证据才重试。生成文件缺失/占用或外部安装出现时，不用 clean、改增量开关、换成 package 或连续安装绕过竞争。补丁前重读共用文件差异，安装前核对源码与 APK 对应关系；场景中途发生外部安装/跳转后，先重新核对版本与场景，受影响证据失效，其他有效阶段保留。恢复协调后只续首个失效/缺失阶段，不扩大修复他人的未完成代码。

小范围文案/样式先按项目规则完成 XML、目标 key 覆盖及差异检查；新增引用最多一次必要资源处理，不为同批语言补齐分次 assemble。数据绑定/换算变化选聚焦逻辑测试或最窄编译，只有确需产物或集成边界才升级构建。

已获准接入截图框架时，先复用项目的已验证配置和运行说明，核对当前 AGP/Kotlin/KSP、插件传递依赖、Jetifier/覆盖率及渲染器/运行桩 API。只处理实际命中的兼容问题，记录原因、限定作用域和验证命令；不把某项目的版本锁定、排除插桩或 API 降级变成通用默认，不改变普通单测/发布构建来迁就截图框架。超出已确认依赖或平台改动范围时确认增量。
