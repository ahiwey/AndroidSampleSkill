...
1. 如果只是修改 1～3 个已存在 key 的 value，先走单 key 快速路径：只定位目标 key、定向替换，并检查覆盖率、变更文件 XML、占位符和差异，不运行全量审计或排序。
2. 其他任务只定位目标模块及其 `src/main/res`，不扫描无关模块，再使用随附脚本执行只读审计，区分结构缺失和语义翻译缺失。
